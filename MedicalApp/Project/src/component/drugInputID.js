...
export default class Druginput extends Component{
    ...
    retrieveinfo() {
        var getinfo = 'http://192.168.0.109:3000/drugReceipt/findinfo';
        var self = this;
        axios.post(getinfo, {
            drugId: this.state.drugId
        }).then(res => {
            self.setState({ID: res.body.ID});
            self.setState({DocID: res.body.Doctor_ID});
            self.setState({Drug_List: res.body.Drug_List});
            self.setState({Price: res.body.Price});
        })
    }
    ...
    sendrequest() {
        ...
        var requrl = 'http://192.168.0.109:3000/api/deleterequest';
                ...
                axios.post(validateurl, {
                    ...
                    if(res.data.success == true) {
                        axios.post(getinfo, {
                            ...
                            if(res){
                                JSON.stringify(res)
                                self.setState({ID: res.data.ID});
                                self.setState({DocID: res.data.Doctor_ID});
                                self.setState({Drug_List: res.data.Drug_List});
                                self.setState({Price: res.data.Price});
                                console.log(res.data)
                                axios.post(requrl, {
                                    id: this.state.citizen_id,
                                    idp : this.state.idp,
                                    request_message: 'I would like to receive my drug list at Pharmarcy1. DrugID: '+this.state.ID+'\n'+'Doctor ID: '+this.state.DocID+'\n'+'Drug Infomation: '+this.state.Drug_List+'\n'+'Price: '+this.state.Price,
                                })
                                .then(res => {
                                    console.log(res);
                                    if(res.data.request_id == null){
                                ...
                                return (ToastAndroid.show('Wrong Drug ID',ToastAndroid.SHORT))
                            }
                        })
                    }
                    else {
    ...
    render() {
        ...
        if (isRequest && this.state.res_req_id != '') {
            responsecard = <View style = {styles.container}>
                        ...
                        <TouchableOpacity style={styles.buttonContainer2} onPress={this.tocode}><Text style = {{color: "white"}}>IdP Result</Text></TouchableOpacity>
                    </View>;
        }
                ...
                <SafeAreaView style = {styles.container}>
                    ...
                    >
                     ...
                     <View style = {styles.inputcontainer}>
                                <Text style ={{marginTop:10, marginLeft: 10}}>Drug Information ID</Text>   
                                         <TextInput 
                                             placeholder = "ex. 00001"
                                             placeholderTextColor = 'gray'
                                             style = {styles.input}
                                             onChangeText={(value) => this.setState({drugId: value})}
